...
void updateDistanceThreshold()
{
	...
	{
		target_dist_thr_ = 1.8*mean_target + 1.5*stdev_target;
...
void callbackNoFeatCarac(const sensor_msgs::ImageConstPtr &image_msg, const opt_msgs::TrackArray::ConstPtr &tracks_msg, const opt_msgs::DetectionArray::ConstPtr &detections_msg)
{
	...
	else // !init_done_flag
	{
		...
		if (idx >= 0) //Si no se ha perdido el track
		{
			...
			if (findTargetRect(image, track_array, detection_array, r_target, rects) && (!feat_only_start || nro_start<nro_only_start))
			{   
		...
		else //Se perdio el track, hay que encontrar al target dentro de las demas detecciones
		{
			...
			for (int i = 0; i < tracks_msg->tracks.size(); i++)
			{
				...
				if (findTrackRect(msg_track, det_arr, rect_target))
				{
					...
					if (hist_distractors_.size() < max_distractors_)
					{
						if (abs(dist_target_track2) > 2.5*target_dist_thr_)
						{
...
int main(int argc, char **argv)
{
	...
	typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, opt_msgs::TrackArray, opt_msgs::DetectionArray> MySyncPolicy;

	message_filters::Synchronizer<MySyncPolicy> sync2(MySyncPolicy(10), sub_image_, sub_tracks_, sub_detect_);
	sync2.registerCallback(boost::bind(&callbackNoFeatCarac, _1, _2, _3));
	
	ROS_INFO("Follower listo");

	namedWindow("Follower");
	namedWindow("Reencontrado");
	namedWindow("Muestra negativa");
	moveWindow("Muestra negativa", 800,20);
	moveWindow("Reencontrado", 600,20);
